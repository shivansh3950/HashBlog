...
const handleSubmit=(e)=>{
  ...
  const fetchDetails=async ()=>{
    const res=await axios.get(`http://localhost:8888/api/blog/${id}`).catch(err=>console.log(err));
    const data =await res.data;
    return data;
  };
  useEffect(() => {
    fetchDetails().then(data=>{setBlog(data.blog);setInputs({title:data.blog.title,description:data.blog.description})
    });
  },[id]);
  const sendRequest=async()=>{
    const res=axios.put(`http://localhost:8888/api/blog/update/${id}`,{
          title:inputs.title,
